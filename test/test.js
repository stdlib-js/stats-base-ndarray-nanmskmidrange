...
tape( 'the function has an arity of 1', function test( t ) {
	t.strictEqual( nanmskmidrange.length, 1, 'has expected arity' );
	t.end();
});

tape( 'the function calculates the mid-range of a one-dimensional ndarray according to a mask, ignoring NaN values', function test( t ) {
	var mask;
	var x;
	var v;
	...
	mask = [ 0, 0, 0, 1, 0, 0, 0 ];
	v = nanmskmidrange( [ vector( x, x.length, 1, 0 ), vector( mask, mask.length, 1, 0 ) ] );
...
tape( 'if provided an empty ndarray, the function returns `NaN`', function test( t ) {
	...
	v = nanmskmidrange( [ vector( x, 0, 1, 0 ), vector( mask, 0, 1, 0 ) ] );
	t.strictEqual( isnan( v ), true, 'returns expected value' );

	t.end();
});

tape( 'if provided an ndarray containing a single element, the function returns the element value', function test( t ) {
	var mask;
	var x;
	var v;
	...
	v = nanmskmidrange( [ vector( x, 1, 1, 0 ), vector( mask, 1, 1, 0 ) ] );
...
tape( 'the function supports one-dimensional ndarrays having non-unit strides', function test( t ) {
	...
	v = nanmskmidrange( [ vector( x, 5, 2, 0 ), vector( mask, 5, 2, 0 ) ] );
...
tape( 'the function supports one-dimensional ndarrays having negative strides', function test( t ) {
	...
	v = nanmskmidrange( [ vector( x, 5, -2, 8 ), vector( mask, 5, -2, 8 ) ] );
...
tape( 'the function supports one-dimensional ndarrays having non-zero offsets', function test( t ) {
	...
	v = nanmskmidrange( [ vector( x, 5, 2, 1 ), vector( mask, 5, 2, 1 ) ] );
	t.strictEqual( v, 1.0, 'returns expected value' );
